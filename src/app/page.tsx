...
export default function Home() {
  ...
  if (!appStarted) {
    return (
      <div className="min-h-screen bg-primary-dark overflow-hidden flex flex-col relative selection:bg-accent-indigo selection:text-white">
        {/* Floating Nav */}
        <nav className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 modern-card px-4 sm:px-8 py-3 sm:py-4 rounded-full flex items-center justify-between w-[95%] max-w-7xl">
        ...
        {/* Hero Content */}
        <div className="flex-1 w-full flex flex-col justify-center sm:justify-end px-5 sm:px-12 md:px-24 pb-16 sm:pb-24 max-w-[1400px] mx-auto z-10 pt-28 sm:pt-40">
          <div className="max-w-5xl">
            <h1 className="text-5xl sm:text-[6rem] md:text-[9rem] leading-[0.9] text-secondary-slate uppercase tracking-tighter mb-6 sm:mb-8 flex flex-col">
            ...
            <p className="text-base sm:text-lg md:text-2xl font-sans text-text-secondary mb-10 sm:mb-16 max-w-2xl leading-relaxed">
              Raw precision generation. No abstractions
            </p>
            <button
              onClick={() => setAppStarted(true)}
              className="button-magnetic bg-secondary-slate hover:bg-black text-white px-6 sm:px-10 py-4 sm:py-6 rounded-smooth font-bold font-sans-tight text-lg sm:text-xl md:text-2xl flex items-center w-max gap-4 sm:gap-6 transition-all shadow-lg mb-6"
            >
  ...
  return (
    <div className="min-h-screen transition-colors bg-primary-slate">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 max-w-7xl mx-auto gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            <button
               ...
               className="text-xs sm:text-sm font-sans text-secondary-slate hover:text-accent-indigo transition-colors uppercase px-2 sm:px-4 font-semibold"
            >
              [ ABORT ]
            </button>
            <button
              onClick={() => setShowAdminModal(true)}
              className="button-magnetic px-3 sm:px-5 py-2 rounded-smooth bg-secondary-slate hover:bg-black text-white font-semibold font-sans-tight transition-all min-w-[80px] sm:min-w-[100px] uppercase text-xs sm:text-sm shadow-md"
            >
              {isAdmin ? 'SYS_ADMIN' : 'SYS_LOGIN'}
            </button>
          </div>
      ...
      <main className="pt-16 sm:pt-24 pb-12 min-h-screen max-w-7xl mx-auto px-3 sm:px-4">
        <InvoiceGenerator />
      </main>
